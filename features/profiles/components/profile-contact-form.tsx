...
export function ProfileContactForm({ accountEmail, contact }: ProfileContactFormProps) {
  ...
  return (
    <Paper withBorder radius="md" p={0} className={styles.contactCard}>
      <form action={formAction}>
        <Stack gap={0}>
          <header className={styles.contactHeader}>
            <div>
              <Text className={styles.contactEyebrow}>Private handoff</Text>
            ...
            <div className={styles.privacyNote}>
              ...
              Контакты закрыты от каталога и становятся доступны только второй стороне принятой заявки.
            </div>
          </header>
          ...
          <div className={styles.contactBody}>
            ...
            <div className={styles.sharingControl}>
              ...
              {fieldError("sharing_enabled") && (
                <Text size="xs" c="red" mt="xs">{fieldError("sharing_enabled")}</Text>
              )}
            </div>
          </div>
